...
def analyze_oov(word: str, lemma_chapter_map: dict) -> dict:
    ...
    if lemma in lemma_chapter_map:
        # 词条原形已在映射表中 → evaluate_v2 应该能匹配，可能没匹配
        return {
            "word": word,
            "clean": clean,
            "lemma": lemma,
            "verdict": "lemma_in_map",
            "actual_chapter": lemma_chapter_map[lemma],
            "suggestion": f"lemma '{lemma}' 已在映射表（Cap.{lemma_chapter_map[lemma]}），可能是 evaluate_v2 漏匹配，不动映射表",
            "action": "none",
        }
    ...
    neighbors = [
        ...
        if k.startswith(prefix4) and k != lemma
    ]
    if neighbors:
        ...
        return {
            ...
            "suggestion": f"lemma '{lemma}' 不在表里，近邻词（如 '{neighbors[0][0]}'）属于 Cap.{min_chapter}",
            "action": "suggest_add",
    ...
    return {
        ...
        "suggestion": f"lemma '{lemma}' 不在表里，且无近邻词，需要人工判断章节归属",
        "action": "needs_human",
    }
...
def main() -> int:
    ...
    for word, freq in oov_counter.most_common():
        if freq < args.min_frequency:
            ...
            continue
        result = analyze_oov(word, lemma_chapter_map)
